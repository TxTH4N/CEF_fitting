...
def err_global(CFLevelsObject, coeff, prefactor):
    ...
    for idx, t in enumerate(t_list):
        sigsq += np.sum((prefactor * spectrum(CFLevelsObject,x_e_12,Temp=t,ResFunc=ins_res_12,gamma=gamma_12[idx]) - int_12_list[idx]) ** 2/un_12_list[idx])
        sigsq += np.sum((prefactor * spectrum(CFLevelsObject, x_e_3, Temp=t, ResFunc=ins_res_3p32, gamma=gamma_3[idx]) -  1/rat*int_3p32_list[idx]) ** 2/un_3p32_list[idx])
...
print(TVS.B)
## Fit to neutron data
FitCoefRes1 = TVS.fitdata(chisqfunc=err_global, fitargs=['prefactor','coeff'],method= 'SLSQP',
...
for idx, t in enumerate(t_list):
    ...
    axes.errorbar(x_e_3, 1/rat*int_3p32_list[idx],yerr=1/rat*un_3p32_list[idx], marker='.', ls='none', label='data', color="C{}".format(idx))
