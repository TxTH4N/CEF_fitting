...
for B66 in B66List:
    ...
    # eigenval_0, eigenvec_0 = np.linalg.eig(TVS.H)
    # normal_eigenval_0 = np.abs(min(eigenval_0)) + eigenval_0
    # print(np.sort(eigenval_0))
    ax3.plot(xx, sim_int_40, label='0 T', marker='.', markersize=2)

    print("After field:")
    # print(TVS.H)
    field_H = TVS.H+g*muB*B*mJ
    TVS.H += g * muB * B * mJ
    TVS.diagonalize()#Hamiltonian=field_H)
    print(TVS.eigenvalues)
    # eigenval,eigenvec = np.linalg.eig(field_H)
    # normal_eigenval = np.abs(min(eigenval))+eigenval
    # print(np.sort(normal_eigenval))
    sim_int_40_field = 0.0005 * TVS.normalizedNeutronSpectrum(xx, Temp=30, ResFunc=ins_res_3p32)
    ax3.plot(xx, sim_int_40_field, label='0.1 T', marker='.',markersize=2)
