...
ax2.errorbar( Y1_common[:-9],Y2_interp[:-9],yerr=0, marker='o',linestyle='None')
log_2d = np.log(Y2_interp[:-9]-delta_0)
log_2d_err = np.sqrt(Y2_err_interp[:-9]**2+delta_0_err**2)/Y2_interp[:-9]
log_mag = np.log(Y1_common[:-9])
log_mag_err =Y1_common_err[:-9]/Y1_common[:-9]
...
ax3.plot(log_mag, result.best_fit, linestyle='-', label='best fit:0.6X-2.99')
# ax3.plot(log_mag, result.best_fit, linestyle='-', label='best fit:0.77X-3.27')
ax3.plot(log_mag,0.5*log_mag-2.83729,linestyle='--', label='0.5X-2.84')
# ax3.plot(log_mag,log_mag-3.63,linestyle='--', label='X-3.63')
ax3.grid(alpha=0.6)
...
ax4.errorbar( Y1_common[:-9],Y2_interp[:-9],xerr=Y1_common_err[:-9],yerr = Y2_err_interp[:-9], marker='o',linestyle = 'None')
ax4.set_xscale('log', base=np.e)
ax4.set_yscale('log', base=np.e)

# Optional: Format ticks as e^n
from matplotlib.ticker import FuncFormatter
formatter = FuncFormatter(lambda val, _: f"$e^{{{np.log(val):.0f}}}$")
ax4.xaxis.set_major_formatter(formatter)
ax4.yaxis.set_major_formatter(formatter)

ax4.set_xlabel("X")
ax4.set_ylabel("Y")
ax4.set_title("Uniformly Distributed Log-Log Plot (base e)")
