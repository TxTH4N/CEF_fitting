...
g = 3 / 2
#     obj.H += g * muB  * mJ * field
def err_global(CFLevelsObject, coeff, prefactor,field):
    print(field)
    # coeff[-1]=B66
    CFLevelsObject.newCoeff(coeff)
    CFLevelsObject.H += g * muB  * mJ * field
    ...
    for idx, t in enumerate(t_list):
        ...
        sigsq += rat*np.sum((prefactor * spectrum(CFLevelsObject, x_e_3, Temp=t, ResFunc=ins_res_3p32, gamma=gamma_3[idx]) - int_3p32_list[idx]) ** 2)
...
print(TVS.B)
## Fit to neutron data
FitCoefRes1 = TVS.fitdata(chisqfunc=err_global, fitargs=['prefactor','coeff','field'],
                        prefactor=prefactor, coeff=TVS.B,field=field_guess)
print(FitCoefRes1)
...
TVS.newCoeff(FitCoefRes1['coeff'])
TVS.H += g * muB * FitCoefRes1['field'] * mJ
TVS.diagonalize()  # Hamiltonian=field_H)
chisq = 0
for idx, t in enumerate(t_list):
    ...
    FittedSpectrum2 = rat * spectrum(TVS,x_e_3, Temp=t,ResFunc=ins_res_3p32,gamma=gamma_3[idx]) * FitCoefRes1['prefactor']
